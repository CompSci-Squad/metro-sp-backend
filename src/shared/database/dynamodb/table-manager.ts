...
export abstract class DynamoDBTableManager {
    ...
    protected async ensureTableExists(
    ): Promise<void> {
        try {
            await this.dbClient.send(new DescribeTableCommand({ TableName: this.tableName }));
            this.logger.log(`Table ${this.tableName} already exists.`);
        } catch (error) {
            if (error.name === "ResourceNotFoundException") {
                this.logger.warn(`Table ${this.tableName} does not exist. Creating now...`);
    ...
    protected handleError(error: any, action: string): void {
        ...
        throw new InternalServerErrorException(`Failed to ${action}: ${error.message}`);
    }
}
